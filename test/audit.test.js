...
test('API: write routes record audit events with actor, target and request context; denials too', async (t) => {
  ...
  const res = await app.inject({ method: 'POST', url: '/v1/collections', headers: { ...bearer(WRITE_KEY), 'user-agent': 'test-agent' }, payload: { name: 'branches' } });
  assert.ok(res.statusCode < 300, res.body);
  assert.equal(audit.buffer.length, 1);
  const e = audit.buffer[0];
  assert.deepEqual([e.action, e.outcome, e.actor, e.target, e.userAgent, typeof e.requestId, typeof e.id, e.ip], ['geo.collection.create', 'success', { type: 'apikey', id: e.actor?.id }, { type: 'collection', id: 'branches' }, 'test-agent', 'string', 'string', '127.0.0.1']);
  assert.ok(e.actor?.id);
  const denied = await app.inject({ method: 'POST', url: '/v1/collections', headers: bearer(READ_KEY), payload: { name: 'x' } });
  assert.equal(denied.statusCode, 403);
  assert.deepEqual([audit.buffer.length, audit.buffer[1].action, audit.buffer[1].outcome], [2, 'geo.collection.create', 'denied']);
